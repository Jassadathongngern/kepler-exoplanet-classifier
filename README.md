...
Transit Signatures: koi_period (Orbital period), koi_duration (Transit duration), koi_depth (Transit depth), koi_prad (Planetary radius).

Stellar Properties: koi_model_snr (Signal-to-Noise), koi_teq (Equilibrium Temp), koi_srad (Stellar Radius), koi_steff (Stellar Temp).

Advanced Physics: koi_slogg (Surface Gravity), koi_insol (Insolation Flux), koi_kepmag (Kepler Magnitude).
